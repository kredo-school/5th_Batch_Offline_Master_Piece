            ...
            <div id="comments-section" class="col-10">
                <div class="card w-100 mb-5">
                    <div class="card-header bg-white border-bottom-0">
                        <div class="text-end">
                            <a href="{{ url()->current() . '?page=' . $latestPage . '#comment-' . count($thread->comments) }}"
                                class="text-secondary">Latest comment <i class="fa-solid fa-arrow-down"></i></a>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-auto">
                                <div class="justify-content-end d-flex">
                                    ...
                                    @can('admin')
                                        <button class="btn btn-danger border-0" data-bs-toggle="modal"
                                            data-bs-target="#delete-thread-{{ $thread->id }}">
                                            Delete thread
                                        </button>
                        ...
                        @foreach ($comments->take(100) as $comment)
                            ...
                            <div class="row">
                                <div class="col-10 fs-5 comment" data-id="{{ $comments->firstItem() + $loop->index }}">
                                    <p>{{ $comments->firstItem() + $loop->index }} name:
                                        @if ($comment->user)
                                            <a href="{{ route('profile.show', $comment->guest_id) }}"
                                        ...
                                        @if (!$comment->deleted_at)
                                            <a href="#add-comment"
                                                onclick="setParentId({{ $comments->firstItem() + $loop->index }})"
                                ...
                                <div class="col-2 text-end">
                                    ...
                                    @if (Auth::user()->id !== $comment->guest_id)
                                        ...
                                        @else
                                            <button class="btn border-0" data-bs-toggle="modal"
                                                data-bs-target="#report-comment-{{ $comment->id }}">
                                                <div class="fs-24">
                            ...
                            <div class="px-4 fs-24" id="comment-{{ $comments->firstItem() + $loop->index }}"
                                ...
                                @if ($comment->parent_id)
                                    ...
                                    <a href="{{ url()->current() . '?page=' . $page . '#comment-' . $comment->parent_id }}"
                                        class="text-decoration-none">>>{{ $comment->parent_id }}</a>
                                @endif
                ...
                <div class="card">
                    <form action="{{ route('comment.addComment', $thread) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="card-header bg-white p-0">
                            <textarea name="body" id="add-comment" rows="3" placeholder="Add comment"
                                class="form-control rounded-bottom-0 bg-white border-0 fs-24"></textarea>
